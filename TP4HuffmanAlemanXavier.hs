...
insere (Leaf (x1,y1)) ((Leaf (x2,y2)):huffs) | y1 <= y2 = ((Leaf (x1,y1)):(Leaf (x2,y2)):huffs)
                                             | otherwise = ((Leaf (x2,y2)):insere (Leaf (x1,y1)) huffs)
insere huff [] = huff:[]
...
triHuff huffs = foldr (insere) [] huffs
...
agrege [x] = x
agrege (x1:x2:xs) | poids x1 <= poids x2 = agrege ((Node x1 x2):xs)
                  | otherwise = agrege ((Node x2 x1):xs)
